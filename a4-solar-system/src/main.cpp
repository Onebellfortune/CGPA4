...
#include "trackball.h"
//#include "trackball2.h"	// virtual trackball
#include "circle.h"
...
//*************************************
// global constants
static const char*	window_name = "A4 - Full Solar System";
...
bool	b_index_buffer_circle = true;
...
auto	gories = std::move(create_circles(NUM_GORIES));
uint	mode = 0;
uint	k = 0;
//*************************************
// light objects
...
void update()
{
	...
	glUniform1i(glGetUniformLocation(program, "SATURN_RING"), 9);
...
std::vector<vertex> update_vertex_vertices(uint N)
{
	std::vector<vertex> v = { { vec3(0,0,0), vec3(0,0,-1.0f), vec2(0.5f) } }; // origin
	//printf("%lf\n", r);
	for (uint k = 0; k <= N; k++)
	{
		float p = PI * 2.0f * k / float(N), cp = cos(p), sp = sin(p);//pi=longitude
		v.push_back({ vec3(cp,sp,0.0f), vec3(0.0f,0.0f,-1.0f), vec2(0.5f + 0.5f * p) });

	}
	return v;
}

void update_vertex_buffer_circle(const std::vector<vertex>& vertices, uint N)
{
	static GLuint vertex_buffer_circle = 0;	// ID holder for vertex buffer
	static GLuint index_buffer_circle = 0;		// ID holder for index buffer

	// clear and create new buffers
	if (vertex_buffer_circle)	glDeleteBuffers(1, &vertex_buffer_circle);	vertex_buffer_circle = 0;
	if (index_buffer_circle)	glDeleteBuffers(1, &index_buffer_circle);	index_buffer_circle = 0;

	// check exceptions
	if (vertices.empty()) { printf("[error] vertices is empty.\n"); return; }

	// create buffers
	if (b_index_buffer_circle)
	{
		std::vector<uint> indices;
		for (uint k = 0; k < N; k++)
		{
		...
		// generation of vertex buffer: use vertices as it is
		glGenBuffers(1, &vertex_buffer_circle);
		glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer_circle);
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertex) * vertices.size(), &vertices[0], GL_STATIC_DRAW);

		// geneation of index buffer
		glGenBuffers(1, &index_buffer_circle);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index_buffer_circle);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint) * indices.size(), &indices[0], GL_STATIC_DRAW);
	}
	else
	{
	...
	// generate vertex array object, which is mandatory for OpenGL 3.3 and higher
	if (vertex_array_gori) glDeleteVertexArrays(1, &vertex_array_gori);
	vertex_array_gori = cg_create_vertex_array(vertex_buffer_circle, index_buffer_circle);
	if (!vertex_array_gori) { printf("%s(): failed to create vertex aray\n", __func__); return; }
}
...
bool user_init()
{
	...
	solar_system = std::move(create_sphere_vertices(NUM_TESS));
	gori = std::move(update_vertex_vertices(NUM_TESS));
